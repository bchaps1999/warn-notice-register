...
def raw_tx(tmp_path_factory):
    path = tmp_path_factory.mktemp("tx-source") / "tx.csv"
    with tarfile.open(BUNDLE, "r:gz") as archive:
        source = archive.extractfile("raw/tx.csv")
        assert source is not None
        path.write_bytes(source.read())
    return path


def test_texas_workbooks_pin_all_source_rows_and_republished_year(raw_tx):
    rows = read_artifacts(ARTIFACTS, raw_tx)
...
def test_texas_workbook_hash_header_and_raw_correspondence_fail_closed(tmp_path, raw_tx):
    ...
    with pytest.raises(ValueError, match="checksum mismatch"):
        read_artifacts(directory, raw_tx)
    (directory / "2026.xlsx").write_bytes(original)
    raw = tmp_path / "tx.csv"
    raw.write_bytes(raw_tx.read_bytes().replace(b"Temco Logistics", b"Temco LogisticS", 1))
...
def test_texas_date_annotation_requires_current_raw_and_canonical_match(tmp_path, raw_tx):
    norm = normalize_file("tx", raw_tx.parent, load_registry()["tx"].source_url)
    ...
    try:
        ...
        exceptions = []
        report = apply_date_evidence(conn, ARTIFACTS, raw_tx, "2026-09-22", exceptions)
...
def test_texas_date_annotation_holds_changed_rows_under_one_old_key(tmp_path, raw_tx):
    norm = normalize_file("tx", raw_tx.parent, load_registry()["tx"].source_url)
